...
function serializeElementNode(n, options) {
  ...
  if (tagName === "img" && inlineImages) {
    ...
    const tryWithCorsImage = (imgSrc) => {
      ...
      corsImage.onload = () => {
        const result2 = copyImageToDataURL(corsImage);
        if (typeof result2.dataURL === "string") {
          attributes.rr_dataURL = result2.dataURL;
        } else {
          console.warn(
    ...
    const recordInlineImage = () => {
      ...
      if (imageSrc == null ? void 0 : imageSrc.startsWith("data:")) {
        attributes.rr_dataURL = imageSrc;
      } else {
        const result2 = copyImageToDataURL(image);
        if (typeof result2.dataURL === "string") {
          attributes.rr_dataURL = result2.dataURL;
        } else if (result2.isSecurityError && image.crossOrigin !== "anonymous") {
          tryWithCorsImage(imageSrc);
        } else if (result2.err) {
          console.warn(`Cannot inline img src=${imageSrc != null ? imageSrc : "<unknown-src>"}! Error:`, result2.err);
        }
      }
    };
